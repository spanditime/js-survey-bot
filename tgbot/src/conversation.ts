...
const GoToSurvey = `На данный момент набор окончен. Однако вы можете оставить заявку, если найдется специалист - он c Вами свяжется. Хотите оставить заявку?`;
...
const EnterContact = 'Как мы можем связаться с Вами? Просим оставить Вас ссылку на соц. сети, почту или номер телефона (и предпочтительный тип связи по нему). Оставить ссылку на телеграмм Вы можете с помощью кнопки ниже (при ее наличии).'
...
`;
  return createOptionsQuestionMapped(
    ...
    new Map<string, (current:SurveyConversationHandler<Pupil>,ctx:ConversationContext)=>void>([
	  ...
	  current.data.timestamp = new Date().toUTCString();
